...
// TO DO: complete this function by implementing the entry and exit sections
{
    ...
    if(board->reader_in_cs==1)  sem_wait(&board->wrt);
    ...
    AddReaderInCS(board, -1);
    if(board->reader_in_cs==1)  sem_post(&board->wrt);
}
...
// TO DO: implement this function.
{
    repeat = 0;
    int check_value;
    sem_getvalue(&board->wrt, &check_value);
    if(check_value == 0){
        sem_post(&board->wrt);
    }
